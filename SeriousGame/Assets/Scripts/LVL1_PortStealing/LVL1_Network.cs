...
public class LVL1_Network : MonoBehaviour {
    ...
    [SerializeField] LVL1_Station alice;
    [SerializeField] LVL1_Station bob;
    [SerializeField] LVL1_Station mallory;
    [SerializeField] LVL1_Station hub;

    SortedDictionary<string, LVL1_Station> stations = new SortedDictionary<string, LVL1_Station>();
    ...
    public List<string> GetIPList() {
        ...
        foreach (LVL1_Station s in stations.Values) {
            list.Add(s.GetIP());
        }
        return list;
    }
    ...
    public string GetKeyByIP(string IP) {
        foreach (KeyValuePair<string, LVL1_Station> s in stations) {
            if (s.Value.GetIP() == IP) {
                return s.Key;
            }
        }
    ...
    public LVL1_Notes GetNotes() {
        return notes.GetComponent<LVL1_Notes>();
    }
    ...
    public void BeginLevel() {
        alice.GetComponent<LVL1_Alice>().BeginLevel();
        bob.GetComponent<LVL1_Bob>().BeginLevel();
        Time.timeScale = 1;
    }
}
